...
final class AttemptDebugSnapshotTests: XCTestCase {
    func testBuildsSummaryFromAcceptedPitchFrames() throws {
        let frames = [
        ...
        XCTAssertEqual(try XCTUnwrap(snapshot.firstAcceptedFrameMs), 120, accuracy: 0.001)
        XCTAssertEqual(try XCTUnwrap(snapshot.medianDetectedHz), 440, accuracy: 0.001)
        XCTAssertEqual(try XCTUnwrap(snapshot.meanConfidence), 0.90, accuracy: 0.001)
        XCTAssertEqual(try XCTUnwrap(snapshot.meanRMS), 0.20, accuracy: 0.001)
    }
